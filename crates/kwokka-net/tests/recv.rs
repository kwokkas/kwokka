...
//! then drives the public [`TcpStream::recv`] and [`TcpStream::recv_buf`]
//! entries on the adopted server end through the real `io_uring` ring: submit
//! a recv, park, harvest the CQE, wake, and read the bytes back. The socket
//! counterpart of the buffered-read e2e test. The write buffers into the
//! server's receive queue, so the recv harvests it sequentially with no
//! concurrent peer.
//!
//! [`TcpStream::recv`]: kwokka_net::tcp::TcpStream::recv
//! [`TcpStream::recv_buf`]: kwokka_net::tcp::TcpStream::recv_buf
...
#[test]
fn recv_buf_returns_sent_bytes() {
    let Ok(listener) = TcpListener::bind("127.0.0.1:0") else {
        panic!("binding a loopback listener must succeed");
    };
    let Ok(addr) = listener.local_addr() else {
    ...
    let payload = b"kwokka generic socket recv";
    ...
    let server = kwokka_net::tcp::TcpStream::from(server);
    let (result, buf) = runtime.block_on(server.recv_buf([0u8; 64]));
    ...
    assert_eq!(
        &buf[..received],
        &payload[..],
        "the caller-owned buffer holds the bytes the peer sent",
    );
}
